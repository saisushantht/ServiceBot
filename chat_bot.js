...
const MongoClient = require('mongodb').MongoClient; console.log("connected");
...
async function identify_user(agent)
{
  ...
  const snap = await client.db("serviceBot").collection("user").findOne({acct_num: acct_num});
  console.log(snap);
  console.log("snap ^|^");
  if(snap==null){
...
function report_issue(agent)
{
 ...
 // console.log(agent);
  ...
  var dbo = db.db("serviceBot");
	...
	var myobj = { username:u_name, issue:"Net Issues",status:status,time_date:time_date,trouble_ticket:trouble_ticket };
